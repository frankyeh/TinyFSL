...
/*
                 ...
                 unsigned int                 level)
  {
    ...
    char fname[256];
    if (level) {
      if (slices.size() == 0) strcpy(bfname,"EDDY_DEBUG_RAW_GENERAL_TRANSFORM");
      else strcpy(bfname,"EDDY_DEBUG_RAW_GENERAL_TRANSFORM_S2V");
      sprintf(fname,"%s_valid_at_entry_%02d_%04d",bfname,iter,scindx); NEWIMAGE::write_volume(*valid,fname);
    }
    cout << "raw_general_transform: debug version" << endl;
    if (int(defdir.size()) != d.tsize()) {imthrow("NEWIMAGE::raw_general_transform: Mismatch in input. defdir.size() must equal d.tsize()",11);}
    for (int i=0; i<int(defdir.size()); i++) {
      if (defdir[i] < 0 || defdir[i] > 2) {imthrow("NEWIMAGE::raw_general_transform: Mismatch in input. Displacements can only be specified for directions 0,1 or 2.",11);}
    }
    ...
    if (level) {
      sprintf(fname,"%s_valid_before_%02d_%04d",bfname,iter,scindx); NEWIMAGE::write_volume(*valid,fname);
    }
    ...
    if (level) {
      sprintf(fname,"%s_valid_after_%02d_%04d",bfname,iter,scindx); NEWIMAGE::write_volume(*valid,fname);
    }
   ...
   // All done!
    ...
    if (level) {
      sprintf(fname,"%s_valid_at_exit_%02d_%04d",bfname,iter,scindx); NEWIMAGE::write_volume(*valid,fname);
    }
...
/*
          ...
          unsigned int                     level)
  {
    ...
    if (level) {
      if (slices.size() == 0) strcpy(bfname,"EDDY_DEBUG_APPLY_WARP");
      else strcpy(bfname,"EDDY_DEBUG_APPLY_WARP_S2V");
      sprintf(fname,"%s_mask_before_%02d_%04d",bfname,iter,scindx); NEWIMAGE::write_volume(mask,fname);
    }
    raw_general_transform(vin,A,d,defdir,derivdir,slices,Tptr,Mptr,vout,deriv,&mask,scindx,iter,level);
    if (level) {
      sprintf(fname,"%s_mask_after_%02d_%04d",bfname,iter,scindx); NEWIMAGE::write_volume(mask,fname);
    }
  }
  */

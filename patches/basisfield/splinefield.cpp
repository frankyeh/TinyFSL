...
splinefield::splinefield(const std::vector<unsigned int>& psz,
			 ...
			 int                              order,
			 unsigned int                     nthr)
: basisfield(psz,pvxs), _sp(order,ksp), _dsp(3), _nthr(nthr)
{
...
// Copy constructor
splinefield::splinefield(const splinefield& inf) : basisfield(inf), _sp(inf._sp), _nthr(inf._nthr)
{
...
std::shared_ptr<BFMatrix> splinefield::MemEnergyHess(MISCMATHS::BFMatrixPrecisionType   prec) const  // Hessian of membrane energy
{
  ...
  if (prec==BFMatrixFloatPrecision) H = std::shared_ptr<BFMatrix>(new SparseBFMatrix<float>(ncoef,ncoef,irp,jcp,valp));
  else H = std::shared_ptr<BFMatrix>(new SparseBFMatrix<double>(ncoef,ncoef,irp,jcp,valp));
...
std::shared_ptr<MISCMATHS::SpMat<float> > splinefield::BendEnergyHessAsSpMat() const // Hessian of bending energy
{
  ...
  std::shared_ptr<MISCMATHS::SpMat<float> > H = std::shared_ptr<MISCMATHS::SpMat<float> >(new SpMat<float>(ncoef,ncoef,irp,jcp,valp));
  delete [] irp; delete [] jcp; delete [] valp;
                                                  ...
                                                  const std::vector<unsigned int>&   isz) const
{
  ...
  std::shared_ptr<SpMat<float> > j = std::shared_ptr<SpMat<float> >(new SpMat<float>(m,n,irp,jcp,valp));

...
Utilities::Option<bool> help(string("-h,--help"), false,
		             ...
		             false, Utilities::no_argument);
Utilities::Option<bool> invert(string("--invert"), false,
		             string("Make distorted data from undistorted input"),
		             false, Utilities::no_argument);
...
Utilities::Option<string> datatype(string("-d,--datatype"), string(""),
                                   string("Force output data type [preserve char short int float double]."),
                                   false, Utilities::requires_argument);
...
int applytopup()
{
  ...
  if (datatype.set()) {
    if (datatype.value()==string("preserve")) dtypecode = NEWIMAGE::dtype(infnames[0]);
    else if (datatype.value()==string("char")) dtypecode = DT_UNSIGNED_CHAR;
  ...
  if (datatype.set()) save_volume_dtype(ovol,outfname.value(),dtypecode);
  else save_volume(ovol,outfname.value());
                                         ...
                                         std::vector<NEWIMAGE::volume4D<float> >&         scans)
{
  ...
  for (unsigned int c=0; c<collections.NCollections(); c++) {
    ...
    // Add to output
    for (int d=0; d<scans[0].tsize(); d++) {
      NEWIMAGE::ShadowVolume<float> tmp_ovol = ovol[d]; // Needed for New Newimage
      for (int sl=0; sl<scans[0].zsize(); sl++) {
        add_to_slice(tmp_ovol,mu[d][sl],sl,row);
      }
    }
                                         ...
                                         std::vector<NEWIMAGE::volume4D<float> >&         scans)
{
  ...
  for (unsigned int c=0; c<collections.NCollections(); c++) {
    ...
    for (int d=0; d<ovol.tsize(); d++) {
      ...
      // Add to output
      for (int sl=0; sl<scans[0].zsize(); sl++) {
        add_to_slice(tmp_ovol,mu[sl],sl,row);
      }
  ...
  for (unsigned int c=0; c<collections.NCollections(); c++) {
	...
	NEWIMAGE::ShadowVolume<float> tmp_ovol = ovol[d]; // Needed for New Newimage
	  ...
	  for (int i=0; i<N; i++) {
        ...
        add_to_slice(tmp_ovol,mu,sl,row);
      } // End direction
    } // End slice
  } // End collection
                                       ...
                                       NEWIMAGE::interpolation                          interp)
{
  ...
  for (unsigned int i=0; i<scans.size(); i++) {        // Loop over acquistions
    ...
    df[2] = 0.0;
    // Calculate Jacobian for intensity modulation
    NEWIMAGE::volume<float>  jac = tmp;
    NEWIMAGE::volume<char>   tmpmask = mask;
    jac = 0.0;
    try { // Will throw if it is not a topup field (i.e. for example a dual echo fieldmap)
    ...
    catch (const TopupFileIOException& e) {
      NEWIMAGE::volume<float> xcomp = topupfile.FieldAsVolume();
      NEWIMAGE::volume<float> ycomp = xcomp;
      NEWIMAGE::volume<float> zcomp = xcomp;
      xcomp *= (datafile.PhaseEncodeVector(inindices[i]))(1) * datafile.ReadOutTime(inindices[i]) * ovol.xdim();
      ycomp *= (datafile.PhaseEncodeVector(inindices[i]))(2) * datafile.ReadOutTime(inindices[i]) * ovol.xdim();
      zcomp = 0.0;
      volume_deffield_2_jacobian(xcomp,ycomp,zcomp,jac); // Defined in this file
    }
    NEWMAT::Matrix  M = NEWMAT::IdentityMatrix(4);
    if (topupfile.MoveParValid()) M = topupfile.MoveMatrix(inindices[i]);
...
{
  if (!samesize(pxd,pyd) || !samesize(pxd,pzd) || !samesize(pxd,jac)) ApplyTopupException("volume_deffield_2_jacobian:: size mismatch in input images");
  NEWIMAGE::volume<float> xd = pxd / pxd.xdim(); xd.setinterpolationmethod(NEWIMAGE::spline);
  NEWIMAGE::volume<float> yd = pyd / pyd.ydim(); yd.setinterpolationmethod(NEWIMAGE::spline);
  NEWIMAGE::volume<float> zd = pzd / pzd.zdim(); zd.setinterpolationmethod(NEWIMAGE::spline);

  for (int k=0; k<jac.zsize(); k++) {
    for (int j=0; j<jac.ysize(); j++) {
      for (int i=0; i<jac.xsize(); i++) {
	float x = static_cast<float>(i);
	float y = static_cast<float>(j);
	float z = static_cast<float>(k);
	float dxdx=0.0; float dxdy=0.0; float dxdz=0.0;
	float dydx=0.0; float dydy=0.0; float dydz=0.0;
	float dzdx=0.0; float dzdy=0.0; float dzdz=0.0;
	xd.interp3partial(x,y,z,&dxdx,&dxdy,&dxdz);
	yd.interp3partial(x,y,z,&dydx,&dydy,&dydz);
	zd.interp3partial(x,y,z,&dzdx,&dzdy,&dzdz);
	jac(i,j,k) = (1.0 + dxdx)*(1.0 + dydy)*(1.0 + dzdz) + dxdz*dydx*dzdy + dxdy*dydz*dzdx;
	jac(i,j,k) -= (dxdz*(1.0 + dydy)*dzdx + (1.0 + dxdx)*dydz*dzdy + dxdy*dydx*(1.0 + dzdz));
      }
    }
  }
}

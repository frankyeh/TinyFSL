  ...
  : basisfield(psz,pvxs), dctbas(3)
{
  ...
  for (unsigned int i=0; i<3; i++) {
    dctbas[i] = std::vector<std::shared_ptr<NEWMAT::Matrix> >(3);
...
void dctfield::assign_dctfield(const dctfield& inf)
{
  for (int i=0; i<3; i++) {
    dctbas[i] = std::vector<std::shared_ptr<NEWMAT::Matrix> >(3);
    for (int j=0; j<3; j++) {
      dctbas[i][j] = std::shared_ptr<NEWMAT::Matrix>(new NEWMAT::Matrix(*(inf.dctbas[i][j])));
    }
  }
}
...
dctfield& dctfield::operator=(const dctfield& inf)
{
  ...
  dctbas = std::vector<std::vector<std::shared_ptr<NEWMAT::Matrix> > >(3);
...
std::shared_ptr<MISCMATHS::BFMatrix> dctfield::JtJ(const NEWIMAGE::volume<float>&    ima,
                                                     const NEWIMAGE::volume<char>      *mask,
                                                     MISCMATHS::BFMatrixPrecisionType  prec) const
{
  std::shared_ptr<BFMatrix> H = std::shared_ptr<BFMatrix>(new FullBFMatrix);
  return(H);
}

std::shared_ptr<MISCMATHS::BFMatrix> dctfield::JtJ(const NEWIMAGE::volume<float>&       ima1,
                                                     const NEWIMAGE::volume<float>&       ima2,
                                                     const NEWIMAGE::volume<char>         *mask,
                                                     MISCMATHS::BFMatrixPrecisionType     prec) const
{
...
std::shared_ptr<MISCMATHS::BFMatrix> dctfield::JtJ(const std::vector<unsigned int>&       deriv,
                                                     const NEWIMAGE::volume<float>&         ima,
                                                     const NEWIMAGE::volume<char>           *mask,
                                                     MISCMATHS::BFMatrixPrecisionType       prec) const
{
...
std::shared_ptr<MISCMATHS::BFMatrix> dctfield::JtJ(const std::vector<unsigned int>&         deriv1,
                                                     const NEWIMAGE::volume<float>&           ima1,
...
std::shared_ptr<BFMatrix> dctfield::MemEnergyHess(MISCMATHS::BFMatrixPrecisionType prec) const // Hessian of membrane energy
{
  if (prec!=BFMatrixDoublePrecision) throw BasisfieldException("dctfield::MemEnergyHess: Hessian must be double precision");

  std::shared_ptr<FullBFMatrix>     H(new FullBFMatrix);
  NEWMAT::DiagonalMatrix              tmp(CoefSz());
  memen_H(tmp);
  std::shared_ptr<NEWMAT::Matrix>   mp(new Matrix(tmp));   // Wasteful, but can't make it work any other way.
  H->SetMatrixPtr(mp);

  return(H);
}

std::shared_ptr<BASISFIELD::basisfield> dctfield::ZoomField(const std::vector<unsigned int>&    psz,
                                                              ...
                                                              std::vector<unsigned int>           porder) const
{
  ...
  std::shared_ptr<BASISFIELD::dctfield> tptr(new BASISFIELD::dctfield(psz,pvxs,porder));

  const std::shared_ptr<NEWMAT::ColumnVector> ocptr = GetCoef();
  std::shared_ptr<NEWMAT::ColumnVector> ncptr = tptr->get_coef();
...
std::shared_ptr<NEWMAT::Matrix> dctfield::make_dctbas(unsigned int size, unsigned int order, unsigned int deriv) const
{
  std::shared_ptr<NEWMAT::Matrix>  tmptr(new NEWMAT::Matrix(size,order));
...
void dctfield::memen_H(NEWMAT::DiagonalMatrix&  mH) const
{
  ...
  for (int d=0; d<3; d++) {
    ...
    unsigned int csz[] = {CoefSz_x(), CoefSz_y(), CoefSz_z()};

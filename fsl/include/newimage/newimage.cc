...
namespace NEWIMAGE {
 ...
 template <class T>
  ...
  const
  {
    ...
    float     rval = 0.0;
    if (!splint.Valid() || !splineuptodate || translate_extrapolation_type(getextrapolationmethod()) != splint.Extrapolation(0)) {
...
//Shadowvolume implementations
  template <class T>
  int ShadowVolume<T>::initialize(int64_t xsize, int64_t ysize, int64_t zsize, int64_t tsize, int64_t d5, int64_t d6, int64_t d7, T *d, bool d_owner) {
    if ( assigned )
      imthrow("Attempted to reinitialise shadow volume",99);
    int status=volume<T>::initialize(xsize, ysize, zsize, tsize, d5, d6, d7, d, false);
    assigned=true;
    return status;
  }

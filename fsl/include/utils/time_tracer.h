...
namespace Utilities{
  ...
  class Time_Tracer
	  ...
	  if(timingon)
	    {
	      ...
	      std::set<TimingFunction*, TimingFunction::comparer_name>::iterator it = timingFunctions.find(timingFunction);
      ...
      virtual ~Time_Tracer()
	{
      ...
      static void dump_times(const std::string& dir)
	{
	  std::multiset<TimingFunction*, TimingFunction::comparer_time_taken> timingFunctionsByTimeTaken(timingFunctions.begin(), timingFunctions.end());
	  //copy(timingFunctions.begin(), timingFunctions.end(), timingFunctionsByTimeTaken.begin());

	  std::ofstream out;
	  out.open((dir + "/timings.html").c_str(), std::ios::out);
	  out << "<HTML><TITLE>Tracer Timings</TITLE><BODY><table border=3 cellspacing=5>" << std::endl;
	  out << "<tr><td>Function<td align=center>Total Time(secs)<td align=center>Num of calls<td align=center>Time per call(secs)</tr>" << std::endl;
      std::copy(timingFunctionsByTimeTaken.begin(), timingFunctionsByTimeTaken.end(), std::ostream_iterator<TimingFunction*>(out, "\n"));
    ...
    protected:
      ...
      static std::set<TimingFunction*, TimingFunction::comparer_name> timingFunctions;

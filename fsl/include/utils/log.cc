...
namespace Utilities {
  ...
  void Log::makeDir(const string& pdirname, const string& plogfilename, bool pstream_to_logfile, bool pstream_to_cout)
      ...
      if(stream_to_logfile)
	{
	  ...
	  if(logfileout.bad())
	    {
	      throw std::runtime_error(string(string("Unable to setup logfile ")+logfilename+string(" in directory ")+dir).c_str());
	    }
	}
  ...
  void Log::setDir(const string& pdirname, const string& plogfilename, bool pstream_to_logfile, bool pstream_to_cout, std::ios_base::openmode mode)
    {
      if(logEstablished)
	{
	  logfileout.close();
  ...
  void Log::setthenmakeDir(const string& pdirname, const string& plogfilename, bool pstream_to_logfile, bool pstream_to_cout)
      ...
      if(logEstablished)
	{
      ...
      if(ret == -1)
	{
	  throw std::runtime_error(string(string("Unable to make directory ")+dir).c_str());
	}
